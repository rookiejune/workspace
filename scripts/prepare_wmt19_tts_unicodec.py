"""Build a text-preserving UniCodec view for the WMT19 TTS store."""
...
from zhuyin.datasets._wmt19_tts_codec import prepare_unicodec
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    with context():
        ...
        write_json(args.root / "reports" / "prepare_unicodec_summary.json", summary)
...
def run_config(args: argparse.Namespace) -> dict[str, object]:
    return {
        ...
        "write_prefetch": args.write_prefetch,
        "unicodec_cache_dir": (
            None if args.unicodec_cache_dir is None else str(args.unicodec_cache_dir)
        ),
        "domain": args.domain,
        "bandwidth_id": args.bandwidth_id,
        "local_files_only": args.local_files_only,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare a text-preserving UniCodec view for WMT19 TTS."
    )
    ...
    parser.add_argument("--unicodec-cache-dir", type=Path)
    parser.add_argument("--domain", choices=("0", "1", "2"), default="0")
    parser.add_argument("--bandwidth-id", type=int, default=0)
    parser.add_argument(
        "--local-files-only",
